...
import { BottomNav, BootScreen } from "@/components/experiences/mobile/app/bottom-nav";
import { BottomSheet } from "@/components/experiences/mobile/app/bottom-sheet";
import {
  HomeScreen,
  ActivityScreen,
  AccountsScreen,
...
const TAB_ORDER = ["home", "expenses", "accounts", "goals", "profile"];
...
function initialMobileUi(screen?: string) {
  if (screen === "accounts") return { tab: "accounts" };
  if (screen === "goals") return { tab: "goals", goalsTab: "goals" };
  if (screen === "settings") return { tab: "profile", profileTab: "settings" };
  if (screen === "reports") return { tab: "profile", profileTab: "stats" };
  if (screen === "activity" || screen === "transactions") return { tab: "home", homeTab: "activity" };
  if (screen === "income") return { tab: "expenses", expTab: "income" };
  if (screen === "budgets") return { tab: "expenses", expTab: "budget" };
  if (screen === "recurring") return { tab: "expenses", expTab: "recurring" };
  if (screen === "expenses") return { tab: "expenses", expTab: "expenses" };
  return {};
}
...
export function AppShell({ initialData, initialScreen }: { initialData: AppState; initialScreen?: string }) {
  return <AppShellWithTheme initialData={initialData} initialScreen={initialScreen} />;
}

function AppShellWithTheme({ initialData, initialScreen }: { initialData: AppState; initialScreen?: string }) {
  const [theme, setTheme] = useState<SamTheme>(resolveSamTheme(initialData.prefs?.theme));

  return (
    <SamThemeProvider theme={theme}>
      <AppShellInner initialData={initialData} initialScreen={initialScreen} onThemeChange={setTheme} />
    </SamThemeProvider>
  );
}
