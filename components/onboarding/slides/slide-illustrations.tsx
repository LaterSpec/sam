...
export function SlideInvest({ active, slideKey, skipMotion }: SlideProps) {
  const { sam } = useSam();
  const k = active ? slideKey : -1;
  const value = useCountUp(14820, 1100, k, skipMotion);
  const p = useProgress(1600, k, skipMotion);
  const holdings = [
    { sym: "VTI", name: "Total Market", pct: 1.8 },
    { sym: "BTC", name: "Bitcoin", pct: 3.6 },
    { sym: "VXUS", name: "Intl ex-US", pct: -0.4 },
    { sym: "NVDA", name: "NVIDIA", pct: 4.8 },
  ];
  ...
  return (
    ...
    >
      <div>
        ...
        <div style={{ fontSize: 11, color: sam.green, marginTop: 2 }}>▲ 2.4% today · ▲ 18.6% all-time</div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 9 }}>
        <div style={{ fontSize: 12, color: sam.cyan, fontWeight: 600 }}>▸ holdings · live</div>
        {holdings.map((h, i) => {
          const up = h.pct >= 0;
          const show = p > i * 0.12;
          return (
