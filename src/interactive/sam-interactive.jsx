...
// Screen components live in screens/*.jsx; sheets in sheets.jsx (last to load + mounts React)
...
function App() {
  ...
  // Session guard + hydration from Supabase.
  const hydrate = async () => {
    if (!window.SamDB) { setAuthError('backend offline — run `supabase start`'); setLoading(false); return; }
    const session = await window.SamDB.getSession();
    if (!session) { window.SamDB.goOnboarding(); return; }
    const data = await window.SamDB.loadUserData();
    if (!data) { setAuthError('could not load your data'); setLoading(false); return; }
    setState(s => ({ ...s, ...data, selectedGoal: (data.goals[0] && data.goals[0].id) || null }));
    setLoading(false);
  };

  useEffect(() => {
    let alive = true;
    (async () => { if (alive) await hydrate(); })();
    // bounce to onboarding if the session is cleared in another tab
    const { data: sub } = window.SamDB
      ? window.sb.auth.onAuthStateChange((event) => {
          if (event === 'SIGNED_OUT') window.SamDB.goOnboarding();
        })
      : { data: null };
  ...
  useEffect(() => {
    const calc = () => {
      const sx = (window.innerWidth - 40) / 390;
      const sy = (window.innerHeight - 80) / 810;
  ...
  // Apply shell theme CSS variables when theme changes
  useEffect(() => {
    const vars = SHELL_THEME_VARS[state.prefs.theme] || SHELL_THEME_VARS.dark;
    Object.entries(vars).forEach(([k, v]) => document.documentElement.style.setProperty(k, v));
  }, [state.prefs.theme]);

  // Persist preference changes to Supabase (skip the initial hydration).
  useEffect(() => {
    if (loading || !state.user || !window.SamDB) return;
    window.SamDB.updatePrefs(state.prefs);
  }, [state.prefs, loading]);

  // Always-on market poller: refreshes quotes (so portfolio value updates
  // visually on every tab, not just market) and records portfolio-value
  // snapshots, throttled, so the performance chart grows forward from day 0.
  const stateRef = useRef(state);
  stateRef.current = state;
  useEffect(() => {
    if (loading || !state.user || !window.SamDB) return;
    let alive = true;
    const SNAP_MS = 10 * 60 * 1000; // one stored point per ~10 min
    const tick = async () => {
      ...
      setState((prev) => ({
        ...prev,
        market: m,
        portfolioSnapshots: appended ? [...(prev.portfolioSnapshots || []), appended] : prev.portfolioSnapshots,
      }));
    };
    tick();
  ...
  const subTab = state.tab === 'home'     ? state.homeTab
               : state.tab === 'expenses' ? state.expTab
               : state.tab === 'invest'   ? state.investTab
               : state.tab === 'goals'    ? state.goalsTab
               : state.profileTab;

  const prevSubTab = prevTab === 'home'     ? state.homeTab
                   : prevTab === 'expenses' ? state.expTab
                   : prevTab === 'invest'   ? state.investTab
                   : prevTab === 'goals'    ? state.goalsTab
                   : state.profileTab;
  ...
  // Keep the live theme palette + terminal handle in sync with state before
  // any child renders, so SAM.<token> and window.SAM_USER are current.
  ...
  if (loading || authError) {
    return <BootScreen error={authError} onRetry={() => { setAuthError(''); setLoading(true); hydrate(); }}/>;
  }

  return (
    <div data-screen-label="SAM Prototype" style={{
      width: '100vw', height: '100vh',
      ...
      <div style={{
        ...
        <IOSDevice dark={state.prefs.theme !== 'light'} width={390} height={810}>
          <div style={{
            position: 'relative', height: '100%',
            background: 'var(--sam-bg, #0a0e14)', overflow: 'hidden',
          }}>
            <div style={{
              position: 'absolute', top: 54, left: 0, right: 0, bottom: 82,
              overflow: 'hidden',
            }}>
              {transitioning && PrevScreen && (
                <div key={prevTab + '-out'} style={{
              ...
              <div key={state.tab + '-in'} style={{
                position: 'absolute', inset: 0, overflow: 'hidden',
                animation: transitioning
                  ? `slide-in-${animDir > 0 ? 'right' : 'left'} 320ms cubic-bezier(.2,.9,.2,1) forwards`
                  : 'none',
              }}>
                {/* inner layer fades on sub-tab switches (keyed by sub-tab) */}
                <div key={subTab} style={{
                  position: 'absolute', inset: 0, overflowY: 'auto',
                  animation: 'sam-subtab-in 240ms cubic-bezier(.2,.9,.2,1)',
                }}>
                  <ScreenErrorBoundary key={state.tab + ':' + subTab}
                    onReset={() => { setLoading(true); hydrate(); }}>
                    {ActiveScreen && <ActiveScreen state={state} setState={setState} openSheet={setSheet}/>}
                  </ScreenErrorBoundary>
                </div>
              </div>
            </div>
        ...
        <div style={{
          ...
          color: SAM.comment, fontSize: 11,
        }}>
          <Mono c={SAM.textDim} b>SAM</Mono> · tap nav, transactions, goals, categories
        </div>
        <div style={{
          position: 'absolute', top: -34, right: 0,
          color: SAM.comment, fontSize: 11,
          cursor: 'pointer',
        }} onClick={() => { setLoading(true); hydrate(); }}>
          [reload]
        </div>
      </div>
...
// Expose App to window so sheets.jsx can mount it after all screens load
