...
// Mobile-first, terminal aesthetic, animated illustrations
...
function AuthPicker({ onPick, onBack }) {
...
function AuthForm({ mode, onBack, onSubmit, onSwitchMode }) {
  ...
  const submit = async () => {
    ...
    if (!window.SamDB) {
      setSubmitting(false);
      setError('backend offline — run `supabase start`');
      push('✗ supabase unreachable', SAM.red);
      return;
    }
    ...
    if (res.error) {
      push('✗ ' + res.error.message, SAM.red);
...
function App() {
  ...
  const ORDER = ['landing', 'picker', 'login', 'signup', 'done'];
  const goto = (s) => {
    if (s === stage) return;
  ...
  const renderStage = (st) => {
    if (st === 'landing') return <Landing onDone={() => goto('picker')}/>;
    if (st === 'picker')  return <AuthPicker onPick={(m) => goto(m)} onBack={() => goto('landing')}/>;
    if (st === 'login')   return <AuthForm mode="login"  onBack={() => goto('picker')} onSubmit={() => goto('done')} onSwitchMode={() => goto('signup')}/>;
    if (st === 'signup')  return <AuthForm mode="signup" onBack={() => goto('picker')} onSubmit={() => goto('done')} onSwitchMode={() => goto('login')}/>;
    if (st === 'done')    return <Success mode={stage === 'signup' ? 'signup' : 'login'} onContinue={() => {
      // open the interactive prototype
      window.location.href = 'SAM-Interactive.html';
    }}/>;
    return null;
  };
  ...
  return (
    <div data-screen-label="SAM Onboarding" style={{
      ...
      <div style={{
        ...
        <IOSDevice dark={true} width={390} height={810}>
          <div style={{
        ...
        <div style={{
          ...
          <Mono c="rgba(255,255,255,0.7)" b>SAM</Mono> · onboarding
        </div>
        <div style={{
