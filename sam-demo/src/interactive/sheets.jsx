...
// ticker-detail, add-ticker).
import React from 'react';

const { SAM, Mono, Comment, Prompt, BlockBar, BarH, TabBar,
        makeSeries, seriesToPrices, symbolSeed, computeInefficiencies } = window;

function applyInvestAfterTrade(setState, res, holdingsPatch) {
  if (!res || res.error) return res?.error || null;
  setState((s) => {
    const ineff = computeInefficiencies
      ? computeInefficiencies(s.budgets, s.expenses)
      : { byCategory: s.inefficiencies, total: s.inefficienciesTotal };
    return {
      ...s,
      ...holdingsPatch(res),
      investCash: res.investCash != null ? res.investCash : s.investCash,
...
function ExtraSheetContent({ sheet, state, setState, onClose }) {
  ...
  if (sheet.kind === 'trade') {
    ...
    const pct = q.pct || 0;
    const investCash = state.investCash != null ? state.investCash : 0;
    const [mode, setMode] = React.useState('buy');
    const [qty, setQty] = React.useState(1);
    const [tradeErr, setTradeErr] = React.useState('');
    const totalCost = qty * price;
    const canBuy = totalCost > 0 && totalCost <= investCash + 0.001;
    const canSell = totalCost > 0;
    const confirm = async () => {
      if (!(price > 0)) { onClose(); return; }
      setTradeErr('');
      if (mode === 'buy' && !canBuy) {
        setTradeErr('insufficient buying power');
        return;
      }
      if (mode === 'sell' && !canSell) return;
      if (!window.SamDB) { onClose(); return; }
      if (mode === 'buy') {
        const res = await window.SamDB.buyHolding({ symbol: h.sym, name: h.name, amount: totalCost, price });
        const err = applyInvestAfterTrade(setState, res, (r) => {
          if (r.removed) {
            return { holdings: state.holdings.filter((x) => x.sym !== h.sym), watchlist: (state.watchlist || []).filter((w) => w.sym !== h.sym) };
          }
          if (r.row) {
            return {
              holdings: [...state.holdings.filter((x) => x.sym !== h.sym), r.row],
              watchlist: (state.watchlist || []).filter((w) => w.sym !== h.sym),
            };
          }
          return {};
        });
        if (err) { setTradeErr(err); return; }
      } else {
        const res = await window.SamDB.sellHolding({ symbol: h.sym, amount: totalCost, price });
        const err = applyInvestAfterTrade(setState, res, (r) => {
          if (r.removed) return { holdings: state.holdings.filter((x) => x.sym !== h.sym) };
          if (r.row) return { holdings: state.holdings.map((x) => (x.sym === h.sym ? r.row : x)) };
          return {};
        });
        if (err) { setTradeErr(err); return; }
      }
    ...
    return (
      <div>
        ...
        <div style={{ textAlign: 'center', marginTop: 8 }}>
          ...
          <div style={{ fontSize: 11, color: SAM.comment, marginTop: 2 }}>
            {h.qty < 0 ? 'short ' : 'position '}{h.qty} @ avg ${(h.avgCost || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}
          </div>
          <div style={{ fontSize: 11, color: SAM.comment, marginTop: 4 }}>
            {`// disponible: $${investCash.toFixed(2)}`}
          </div>
        </div>
        ...
        {tradeErr && (
          <div style={{ marginTop: 10, fontSize: 12, color: SAM.red, textAlign: 'center' }}>
            {`// ${tradeErr}`}
          </div>
        )}
        ...
        <div onClick={confirm} style={{
          ...
          color: SAM.bg, fontWeight: 700,
          cursor: (mode === 'buy' ? canBuy : canSell) ? 'pointer' : 'default',
          opacity: (mode === 'buy' ? canBuy : canSell) ? 1 : 0.45,
          fontSize: 14,
        }}>
  ...
  if (sheet.kind === 'ticker-detail') {
    ...
    const trade = async (mode) => {
      ...
      setTradeErr('');
      if (mode === 'buy' && amt > investCash + 0.001) {
        setTradeErr('insufficient buying power');
        return;
      }
      if (!window.SamDB) return;
      if (mode === 'buy') {
        const res = await window.SamDB.buyHolding({ symbol: sym, name, amount: amt, price });
        const err = applyInvestAfterTrade(setState, res, (r) => {
          if (r.removed) {
            return { holdings: state.holdings.filter((h) => h.sym !== sym), watchlist: (state.watchlist || []).filter((w) => w.sym !== sym) };
          }
          if (r.row) {
            return {
              holdings: [...state.holdings.filter((h) => h.sym !== sym), r.row],
              watchlist: (state.watchlist || []).filter((w) => w.sym !== sym),
            };
          }
          return {};
        });
        ...
        const err = applyInvestAfterTrade(setState, res, (r) => {
          if (r.removed) return { holdings: state.holdings.filter((h) => h.sym !== sym) };
          if (r.row) return { holdings: state.holdings.map((h) => (h.sym === sym ? r.row : h)) };
          if (r.shortOpened && r.row) {
            return { holdings: [...state.holdings.filter((h) => h.sym !== sym), r.row] };
          }
          return {};
        });
        if (err) { setTradeErr(err); return; }
    ...
    return (
      <div>
        ...
        <div style={{ marginTop: 16 }}>
          ...
          <div style={{ fontSize: 10, color: SAM.comment, marginBottom: 6 }}>
            {`// disponible: $${investCash.toFixed(2)}`}
            {!owned && <Mono c={SAM.red}> · sell abre short</Mono>}
          </div>
          <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <div onClick={() => trade('buy')} style={{
              ...
              cursor: amt > 0 && amt <= investCash + 0.001 ? 'pointer' : 'default',
              opacity: amt > 0 && amt <= investCash + 0.001 ? 1 : 0.4,
          ...
          {tradeErr && (
            <div style={{ marginTop: 8, fontSize: 11, color: SAM.red, textAlign: 'center' }}>
              {`// ${tradeErr}`}
            </div>
          )}
        </div>
        ...
        <div style={{ marginTop: 10, fontSize: 9, color: SAM.comment, textAlign: 'center' }}>
          {`// simulated · buys use buying_power · sells credit cash (short si no hay posición)`}
        </div>
      </div>
    );
  }
...
export function registerExtraSheets() {
  window.ExtraSheetContent = ExtraSheetContent;
}
