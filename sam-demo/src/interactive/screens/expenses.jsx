...
function ExpensesScreen({ state, setState, openSheet }) {
  const cats = (state.budgets || []).map(b => ({
    key: b.key, icon: b.icon, name: b.name, budget: b.cap, c: b.c,
  }));
  const byCat = {};
  state.expenses.forEach(e => { byCat[e.catKey] = (byCat[e.catKey] || 0) + e.amount; });
  const txByCat = {};
  state.expenses.forEach(e => { txByCat[e.catKey] = (txByCat[e.catKey] || 0) + 1; });
  ...
  return (
    <div style={{ padding: '16px 16px 0' }}>
      <TabBar tabs={['expenses', 'income', 'budget']} active={state.expTab}
        onChange={t => setState(s => ({ ...s, expTab: t }))}/>
      <div style={{ marginTop: 20 }}>
        <Prompt host="init.Expenses" cmd="list --month"/>
        <Comment>{totalSpent.toFixed(0)} logged across {state.expenses.length} tx. on pace.</Comment>
          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i}
                ...
                <div style={{ color: isActive ? SAM.yellow : SAM.comment, fontWeight: isActive ? 600 : 400 }}>
                  {d.d}
                </div>
                <div style={{
                  color: isActive ? SAM.yellow : SAM.text,
            ...
            return (
              <div key={cat.key}
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, fontSize: 13 }}>
                  ...
                  <span style={{ flex: 1 }}/>
                  <Mono c={over ? SAM.red : SAM.text} b>${spent.toFixed(0)}</Mono>
                ...
                <div style={{ paddingLeft: 22, marginTop: 3, fontSize: 11, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ color: SAM.comment }}>{pct}% · {txByCat[cat.key] || 0} tx</span>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ marginTop: 20, fontSize: 14 }}>
          <span onClick={() => openSheet({ kind: 'new-expense' })}
